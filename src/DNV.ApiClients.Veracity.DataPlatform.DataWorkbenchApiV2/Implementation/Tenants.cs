...
namespace DNV.ApiClients.Veracity.DataPlatform.DataWorkbenchApiV2
{
    ...
    public partial class Tenants : IServiceOperations<DataWorkbenchApiV2Client>, ITenants
    {
        ...
        /// A response object containing the response body and response headers.
        /// </return>
        public async Task<HttpOperationResponse<IEnumerable<WorkspaceWithTenantReadDto>>> WorkspacesQueryWithHttpMessagesAsync(WorkspaceQueryDto body, Dictionary<string, IList<string>> customHeaders = null, CancellationToken cancellationToken = default)
        {
            if (body == null)
            {
                throw new ValidationException(ValidationRules.CannotBeNull, "body");
            ...
            // Construct URL
            var _baseUrl = Client.HttpClient.BaseAddress?.AbsoluteUri ?? Client.BaseUri.AbsoluteUri;
            var _url = new System.Uri(new System.Uri(_baseUrl + (_baseUrl.EndsWith("/") ? "" : "/")), "workspaces/query").ToString();
            // Create HTTP transport objects
            var _httpRequest = new HttpRequestMessage();
            HttpResponseMessage _httpResponse = null;
            _httpRequest.Method = new HttpMethod("POST");
            ...
            // Create Result
            var _result = new HttpOperationResponse<IEnumerable<WorkspaceWithTenantReadDto>>();
            _result.Request = _httpRequest;
            _result.Response = _httpResponse;
            // Deserialize Response
            if ((int)_statusCode == 200)
            {
                ...
                try
                {
                    _result.Body = Microsoft.Rest.Serialization.SafeJsonConvert.DeserializeObject<IEnumerable<WorkspaceWithTenantReadDto>>(_responseContent, Client.DeserializationSettings);
                }
                catch (JsonException ex)
                {
                    _httpRequest.Dispose();
        ...
        /// The cancellation token.
        /// </param>
        public async Task<IEnumerable<WorkspaceWithTenantReadDto>> WorkspacesQueryAsync(WorkspaceQueryDto body, CancellationToken cancellationToken = default)
        {
            using (var _result = await WorkspacesQueryWithHttpMessagesAsync(body, null, cancellationToken).ConfigureAwait(false))
            {
                return _result.Body;
            }
        }
